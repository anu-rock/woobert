/**
 * Native command-palette integration (spike).
 *
 * Registers an "Ask Woobert" command loader on WordPress core's command palette
 * (`core/commands`, the ⌘K / Ctrl-K bar that WP 7.0 mounts across wp-admin).
 * The loader mirrors the palette's typed text into a single command; selecting
 * it hands the utterance to the standalone Woobert flow modal (flow.js), which
 * runs the resolve/confirm/execute cycle the palette itself can't host.
 *
 * This runs in parallel with the kbar bar (index.js) so we can A/B the two
 * entry points before retiring either.
 */
 ...
 * command. The palette calls this hook with the live `search` text, so this is
 * the native analog of kbar's live searchQuery action.
 */
